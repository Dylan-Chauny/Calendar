...
class CalendarSubscriber implements EventSubscriberInterface
{
    ...
    public function onCalendarSetData(CalendarEvent $calendar): void
    {
        ...
        $this->createCalendarEvent(eventType: $eventTypeHoliday, date: $datePaques);
        ...
        foreach ($datePeriod as $day) {
            ...
            if ($dayOfWeek == 6 or $dayOfWeek == 1)
            {
                ...
                if ($event == self::DAY_OFF_CODE)
                {
                    ...
                    $this->createCalendarEvent(eventType: $eventTypeDayOff, date: $day->format('d-m-Y'));
                }
                else
                {
                    if ($dayOfWeek == 6)
                    ...
                    $this->createCalendarEvent(eventType: $eventTypeDayWork, date: $day->format('d-m-Y'));
                }
            }
        }


        $calendarDates = $this->entityManager->getRepository(\App\Entity\CalendarEvent::class)->findAll();

        foreach ($calendarDates as $calendarDate)
        {
            $calendar->addEvent(new Event(
                            title: $calendarDate->getEventType()->getLabel(),
                            start: new \DateTime($calendarDate->getDate()),
                            options: ['color' => $calendarDate->getEventType()->getColor()]
                        ));
        }
    }
    ...
    public function createHolidaysEvent(EventType $eventType, $day): void
    {
        ...
        {
            $this->createCalendarEvent(eventType: $eventType, date: $day->format('d-m-Y'));
        }
    }
    public function createCalendarEvent(EventType $eventType, $date) {
        $calendarEvent = $this->entityManager->getRepository(\App\Entity\CalendarEvent::class)->findOneBy(['date' => $date]);
